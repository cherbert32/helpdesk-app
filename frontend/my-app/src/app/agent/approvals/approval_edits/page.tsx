...
// defining my form schema for approval reassignment
const schemaReassignemnt = {
    recipient_id:0
};
...
export default function ApprovalDetails() {
    ...
    const [reassignment, setReassignment] = useState(schemaReassignemnt);
    ...
    useEffect(() => {
        // fetching my localstorage for the approval id from the initial users page
        const fetchApproval = async () => {
            ...
            try {
                const response = await fetch(`http://localhost:8000/approvals/agent/${storedApprovalID}`, {
                    method: "GET",
                    headers: {
                ...
                setApproval(data);
                // assigning the reassignment to the corresponding recipient 
                setReassignment({
    ...
    const handleInputChange = (event: any) => {
        ...
        setReassignment(reassignments => ({ ...reassignments, [name]: value }));

    };

    const handleReassignment = async (event: any) => {
        ...
        try {
            const response = await fetch(`http://localhost:8000/approvals/reassign/${approvalID}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${localStorage.getItem("agent_token") || ""}`,
                },
                // making the reassignment into a json format 
            ...
            if (!response.ok) {
                const errorText = await response.text();
                throw new Error(`Reassignment Failed: ${errorText}`);
            }
            // alerts the agent that the approval has been reassigned and closes the pop up
            alert("Reassignment was performed successfully");
            setShowPopUp(false);
            // catches and logs any errors
        } catch (err: any) {
            console.error("Update error:", err);
            alert("An error occurred while attempting to reassign");
        }
    };

    // defining my resubmission function
    const handleResubmission = async () => {
        try {

            const response = await fetch(`http://localhost:8000/approvals/agent/resubmit/${approvalID}`, {
                method: "POST",
                headers: {
            ...
            // alerts the agent that the apporval has been resubmitted and closes the pop up
                ...
                {showPopUp && (
                    <div className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-50">
                        ...
                        >
                            ...
                            {/* created an object that displays the available schema for approval edits similar to a for loop function
                                because some of my schema had underscores, I replaced them with an empty space*/}
                            {Object.keys(reassignment).map((key) => (
