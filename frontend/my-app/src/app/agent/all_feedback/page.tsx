...
export default function AllFeedback() {
    ...
    useEffect(() => {
        const fetchFeedback = async () => {
            try {
                const response = await fetch("http://localhost:8000/feedback/agent/all_feedback", {
                    method: "GET",
                    headers: {
    ...
    const handleViewProfile = (id: number) => {
        ...
        // selecting on the button will take the agent to the next page to view feedback details
        localStorage.setItem("feedback_id", id.toString());
        router.push('/agent/all_feedback/feedback');
    };
