...
// function that will house user notification pop up
export default function UserNotifBar() {
    ...
    // using useEffect to fun once page loads to fetch notifications
    useEffect(() => {
    ...
    const readNotification = async(notification_id: any) => {
        try {
            ...
            const response = await fetch(`http://localhost:8000/notifications/${notification_id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${localStorage.getItem("user_token") || ""}`,
            ...
            setNotifications((currentData) =>
                currentData.map((previous) => (previous.id === notification_id ? {...previous, read: true} : previous
            ))    
            );
        } catch (err) {
                ...
                <div className="mt-2 w-80 bg-white text-black shadow-lg border border-gray-300 rounded p-4">
                    <h3 className="text-lg font-bold mb-2">Your Notifications</h3>
                        ...
                        {notifications.map((notification) => (
                            <div 
                            key={notification.id} 
                            className={`mb-2 border-b pb-2 cursor-pointer ${notification.read? "" : "bg-yellow-100"}`}
